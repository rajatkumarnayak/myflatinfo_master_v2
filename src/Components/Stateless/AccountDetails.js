...
const AccountDetails = ({ amount, status, statusText, items, networkStatus, onUpdate, active }) => {
  ...
  const renderPayButton = () => {
    if (_status === ACCOUNTS_PAYMENT.PENDING.value) {
      ...
      return (
        <Block center>
         <Button
          // onPress={isActive  ? _onUpdate:null} 
          //   round uppercase color={iOSColors.blue} size='small'   style={{backgroundColor : isActive  ? 'green' : 'red'}}
          
           onPress={isActive ? _onUpdate:null} 
            round uppercase color={iOSColors.blue} size='small'  disabled={!isActive} style={{opacity : isActive ? 0.9: 0.3}}
          >
            <Text
