...
const UserClassifiedsScreen = ({navigation, props}) => {
  console.log(props, navigation, 'props at nav');
  const {item} = navigation.state.params;
  console.log(item, 'item >>>>> lsdkfldklfslklsklsk');
  ...
  async function load() {
    ...
    const response = await ApiService.fetchUserClassifiedItemData({
  ...
  const onPressMediaItem = index => {
    console.log( index,'calling >>> images viewer');
  ...
  const _setData = data => {
    ...
    console.log(images, 'images >>> for to show');
...
UserClassifiedsScreen.navigationOptions = ({navigation, screenProps}) => {
...
export default UserClassifiedsScreen;
